...
class MaithiliDataset(Dataset):
            ...
            with open(self.manifest_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if len(parts) >= 2:
                        fname = parts[0]
                        text = parts[1]
                        # Support potential [mai] tag in text or just raw text
    ...
    def __getitem__(self, idx):
        ...
        # Do NOT prepend [START] in text since MTLTokenizer adds [mai] as prefix to the text.
        # This will misorder tokens as [mai] [START] instead of inference which uses [START] [mai].
        # Better to pad the tokens manually.
        text_tokens = self.tokenizer.text_to_tokens(text, language_id='mai', lowercase=False).squeeze(0)
...
def train(args):
    ...
    if rank == 0 and not args.no_wandb:
        wandb.init(project="chatterbox-maithili-finetune")
    ...
    if args.resume_t3_weights:
        ...
        # Handle vocabulary size potentially being different (e.g. if you added [mai])
    ...
    # OPTIMIZATION: Initialize [mai] (Nepali) tag from [hi] (Hindi) tag
    # This prevents the initial gibberish by starting with a related language!
    hi_idx = 722
    mai_idx = 2455
    with torch.no_grad():
        if not args.skip_hi_init and t3.text_emb.weight.shape[0] > mai_idx:
            print(f"🎯 Initializing [mai] tag ({mai_idx}) weights from [hi] tag ({hi_idx})...")
            # 1. Update Embedding
            t3.text_emb.weight[mai_idx] = t3.text_emb.weight[hi_idx].clone()
            # 2. Update Prediction Head
            t3.text_head.weight[mai_idx] = t3.text_head.weight[hi_idx].clone()
        elif args.skip_hi_init:
            print(f"❄️ Skipping [hi] tag initialization as requested. [mai] will be learned from scratch.")
    ...
    # Dataset
    dataset = MaithiliDataset(args.manifest, tokenizer, s3_tokenizer, voice_encoder, device="cpu", wav_dir=args.wav_dir)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if rank == 0 and epoch % args.save_every == 0:
            ckpt_path = f"t3_maithili_epoch_{epoch}.pt"
            ...
            model_to_save.eval()
            with torch.no_grad():
                test_text = "प्रणाम, हम मैथिली बजैत छी। ई तालिम केहन चलि रहल अछि?"
    ...
    if rank == 0:
        ...
        save_file(final_sd, "t3_mtl_maithili_final.safetensors")
        print("Training finished. Saved to t3_mtl_maithili_final.safetensors")
        ...
        if args.push_to_hub:
            try:
                ...
                api.upload_file(
                    path_or_fileobj="t3_mtl_maithili_final.safetensors",
                    path_in_repo="t3_mtl_maithili_final.safetensors",
                    repo_id=args.push_to_hub,
                    token=token
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resume_t3_weights", type=str, help="Path to a t3_maithili_epoch_X.pt file to resume from")
