...
class BalancedMultilingualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Weighted random sampling to ensure exactly 33/33/33 balance
        r = np.random.random()
        if r < 0.333:
            lang = "en"
        elif r < 0.666:
...
def save_checkpoint(epoch, model, args, is_initial=False):
    ckpt_name = f"t3_v5_epoch_{epoch}.pt"
    ...
    if args.push_to_hub and not is_initial:
        try:
            ...
            prev_ckpt = f"t3_v5_epoch_{epoch-1}.pt"
...
def train(args):
    ...
    if not args.no_wandb:
        wandb.init(project="chatterbox-multilingual-v5", config=args)
    ...
    print("⚡ V5 Training Config: Full fine-tuning enabled (30 layers unfrozen) with strict 33/33/33 balanced sampling.")
